...
def test_make_device(new_devices):
    ...
    [NAND1_ID, CLOCK1_ID, D1_ID, I1_ID,
     I2_ID, RC1_ID] = names.lookup(["Nand1", "Clock1", "D1", "I1", "I2", "R1"])
    ...
    new_devices.make_device(RC1_ID, new_devices.RC, 4)
    ...
    assert rc_device.clock_low_period == 4
    assert rc_device.clock_counter == 0
...
@pytest.mark.parametrize("function_args, error", [
    ...
    ("(RC1_ID, new_devices.RC, 5)", "new_devices.NO_ERROR")
])
def test_make_device_gives_errors(new_devices, function_args, error):
    """Test if make_device returns the appropriate errors."""
    names = new_devices.names
    [AND1_ID, SW1_ID, CL_ID, D_ID, X1_ID, X2_ID, RC1_ID
     ] = names.lookup(["And1", "Sw1", "Clock1", "D1", "Xor1", "Xor2", "Rc1"])

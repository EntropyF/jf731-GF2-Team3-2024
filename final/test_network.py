...
@pytest.mark.parametrize("function_args, error", [
    ...
    ("(OR1_ID, I2, OR1_ID, I2)", "network.OUTPUT_PORT_ABSENT"),

    ("(SW1_ID, None, OR1_ID, None)", "network.OUTPUT_TO_OUTPUT"),

    # Switch device does not have port I1, so give OUTPUT_PORT_ABSENT_ERROR
    ...
    # Input first
    ("(OR1_ID, I2, SW2_ID, None)", "network.OUTPUT_PORT_ABSENT"),
...
def test_rc_devices(new_network):
    """Test if execute_network returns the correct output for RC devices.
    """
    network = new_network
    devices = network.devices
    names = devices.names
    ...
    # Make different devices
    [RC1_ID, RC2_ID, RC3_ID] = names.lookup(["Rc1", "Rc2", "Rc3"])
    devices.make_device(RC1_ID, devices.RC, 2)
    devices.make_device(RC2_ID, devices.RC, 3)
    devices.make_device(RC3_ID, devices.RC, 4)

    # Get device outputs, the expression is in a string here so that it
    # can be re-evaluated again after executing devices
    rc1_output = "network.get_output_signal(RC1_ID, None)"
    rc2_output = "network.get_output_signal(RC2_ID, None)"
    rc3_output = "network.get_output_signal(RC3_ID, None)"

    network.execute_network()
    network.execute_network()
    assert [eval(rc1_output), eval(rc2_output), eval(rc3_output)] == [
                HIGH, HIGH, HIGH]

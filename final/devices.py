...
class Devices:
    ...
    get_switch_value(self, device_id): Get the value of a given switch
    ...
    def make_rc(self, device_id, clock_low_period):
        """Make an RC device with the specified period before it falls low.

        clock_low_period is an integer > 0. It is the number of simulation
        cycles before the RC device is LOW.
        """
        self.add_device(device_id, self.RC)
        device = self.get_device(device_id)
        device.clock_low_period = clock_low_period
        self.add_output(device.device_id, output_id=None,
                        signal=self.HIGH)  # initialised to be high on start-up
    ...
    def make_device(self, device_id, device_kind, device_property=None):
        ...
        if self.get_device(device_id) is not None:
            ...
            # Device property is the clock low time > 0
            if device_property is None:
                error_type = self.NO_QUALIFIER
            elif device_property <= 0:
                error_type = self.INVALID_QUALIFIER
